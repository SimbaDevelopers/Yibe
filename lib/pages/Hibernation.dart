...
class _HybernationState extends State<Hybernation> {
  ...
  void getSelectiveConversations() {
    pvtSelectiveStream =
        DatabaseService.instance.getPvtSelectiveConversations();
    profSelectiveStream =
        DatabaseService.instance.getProfSelectiveConversations();
  }
          ...
          builder: (context, snapshot) {
            ...
            if (snapshot.data == null) {
              return Center(
                  child: Container(
                      child:
                          Text('No pvt conversations are added to selective')));
            }
            ...
            if (snapshot.hasData) {
              ...
              if (conversationSnippet.isEmpty) {
                return Center(
                    child: Container(
                        child: Text(
                            'No pvt conversations are added to selective')));
              }
                      ...
                      itemBuilder: (context, i) {
                        if (conversationSnippet[i].messagesCount == 0) {
                          FirebaseFirestore.instance
                              ...
                              .doc(conversationSnippet[i].chatRoomId)
                              .delete();
                          FirebaseFirestore.instance
                              .collection('Users')
                              .doc(UniversalVariables.myPvtUid)
                              .collection('Private_Chats')
                              .doc(conversationSnippet[i].chattingWithId)
                              .delete();
                        }
                        return Column(
                          children: [
                            conversationSnippet[i].chattingWithId.contains('-')
                                ? ConversationSnippetTile(
                                    snippet: conversationSnippet[i],
                                    url: conversationSnippet[i].image)
                                : conversationSnippet[i].image != null
                                    ? ConversationSnippetTile(
                                        snippet: conversationSnippet[i],
                                        url: conversationSnippet[i].image)
                                    : ConversationSnippetTile(
          ...
          builder: (context, snapshot) {
            ...
            if (snapshot.data == null) {
              return Center(
                  child: Container(
                      child: Text(
                          'No prof Conversations are added to selective')));
            }
                  ...
                  ListTile(
                      ...
                      onTap: () {
                        Navigator.of(context)
                            .pushReplacementNamed('pageHandler');
                        HelperFunction.saveHibernationModeAsSharedPreference(
                            false);
                      }),
                  ListTile(
                    title: Text('Settings',
...
class ConversationSnippetTile extends StatefulWidget {
  ...
  @override
  _ConversationSnippetTileState createState() =>
      _ConversationSnippetTileState();
}
      ...
      onDismissed: (direction) {
        widget.snippet.acType == 'Private'
            ? DatabaseService.instance
                .removeThisPvtChatFromSelective(widget.snippet.chattingWithId)
            : DatabaseService.instance
                .removeThisProfChatFromSelective(widget.snippet.chattingWithId);
      },
      child: ListTile(
        onTap: () {
          widget.snippet.acType == 'Private'
        ...
        subtitle: Text(widget.snippet.type == MessageType.Text
            ? widget.snippet.lastMessage.length > 35
                ? widget.snippet.lastMessage.substring(0, 35) + '...'
